  ...
  if (error || !job) {
    return (
      <div className="text-center py-16">
        ...
        <h3 className="text-lg font-semibold text-white mb-2">{error || "Job not found"}</h3>
        <Link href="/jobs">
      ...
      <div className="mb-8">
        ...
        <div className="bg-[#1a1a2e] rounded-2xl border border-white/10 p-6">
          <div className="flex flex-col md:flex-row justify-between md:items-start gap-4">
            ...
            <Button onClick={() => setUploadOpen(true)} className="btn-accent rounded-xl h-11 px-5 flex-shrink-0">
          ...
          {job.requirements && (
            <div className="mt-6 pt-6 border-t border-white/5">
              ...
              <p className="text-sm text-gray-400 whitespace-pre-wrap bg-white/5 rounded-xl p-4 border border-white/5">
                {job.requirements}
              </p>
            </div>
          )}
      ...
      <div>
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            Candidates
            <span className="text-base font-normal text-gray-500">({candidates.length})</span>
          </h2>
        </div>
        ...
        >
          <TabsList className="bg-[#1a1a2e] p-1 rounded-xl border border-white/10">
            <TabsTrigger value="all" className="rounded-lg text-gray-400 data-[state=active]:bg-cyan-500/10 data-[state=active]:text-cyan-400 data-[state=active]:border data-[state=active]:border-cyan-500/20">
              All ({statusCounts.all})
            </TabsTrigger>
            <TabsTrigger value="reviewed" className="rounded-lg text-gray-400 data-[state=active]:bg-cyan-500/10 data-[state=active]:text-cyan-400 data-[state=active]:border data-[state=active]:border-cyan-500/20">
              Reviewed ({statusCounts.reviewed})
            </TabsTrigger>
            <TabsTrigger value="shortlisted" className="rounded-lg text-gray-400 data-[state=active]:bg-cyan-500/10 data-[state=active]:text-cyan-400 data-[state=active]:border data-[state=active]:border-cyan-500/20">
              Shortlisted ({statusCounts.shortlisted})
            </TabsTrigger>
            <TabsTrigger value="rejected" className="rounded-lg text-gray-400 data-[state=active]:bg-cyan-500/10 data-[state=active]:text-cyan-400 data-[state=active]:border data-[state=active]:border-cyan-500/20">
              Rejected ({statusCounts.rejected})
            </TabsTrigger>
            <TabsTrigger value="analyzing" className="rounded-lg text-gray-400 data-[state=active]:bg-cyan-500/10 data-[state=active]:text-cyan-400 data-[state=active]:border data-[state=active]:border-cyan-500/20">
        ...
        {sortedCandidates.length === 0 ? (
          <div className="bg-[#1a1a2e] rounded-2xl border border-white/10">
            <div className="flex flex-col items-center justify-center py-16 px-4">
              ...
              {statusFilter === "all" && (
                <Button onClick={() => setUploadOpen(true)} className="btn-accent rounded-xl">
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                  </svg>
                  Upload First Resume
